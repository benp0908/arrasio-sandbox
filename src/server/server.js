                ...
                if (!e.invuln) {
                    if (e.master.master.team !== this.body.master.master.team) {
                        ...
                        if (Math.abs(e.x - m.x) < range && Math.abs(e.y - m.y) < range) {
                            ...
                            // Sandbox.
                            // ================================================================
                            if (this.body.sandboxId === e.sandboxId ||
                                ['food', 'miniboss'].includes(e.type))
                            {
...
class Gun {
    ...
    fire(gx, gy, sk) {
        ...
        // Sandbox.
        // ===================================================
        o.sandboxId = this.master.sandboxId ? this.master.sandboxId : DefaultSandboxId;
        // ===================================================
...
class Entity {
    constructor(position, master = this) {
        ...
        // Sandbox.
        // ================================================        
        this.sandboxId = master.sandboxId ? master.sandboxId : DefaultSandboxId;
        // ================================================
                    ...
                    case 'k':{    
                        try {
                            ...
                            if (typeof key !== 'string') { 
                                socket.kick('Incorrect sandbox id format.'); 
                                return 1; 
                            }
                            if (key.length > 8) { 
                                socket.kick('Overly-long sandbox id.'); 
                                return 1; 
                            }                                

                            // Sandbox.
                            let sandboxId = parseInt(key);

                            if (isNaN(sandboxId) || sandboxId < 0 || sandboxId > 99999999){
                                socket.kick('Sandbox Id out of range (must be between 0 and 99999999).'); 
                                return 1; 
                            }

                            socket.sandboxId = sandboxId;                                
                            console.log('Sandbox Id:' + socket.sandboxId);
                            // ====================================================
            ...
            const spawn = (() => {
                ...
                return (socket, name) => {
                    ...
                    let body = new Entity(loc);
                        ...
                        // Sandbox.
                        // ============================================================
                        body.sandboxId = socket.sandboxId ? socket.sandboxId : DefaultSandboxId;
                        socket.talk('G', body.sandboxId);
                        // ============================================================
                    ...
                    // Sandbox.
                    // ==============================================================
                    player.teamColor = (!c.RANDOM_COLORS && ['ffa', 'sandbox'].includes(room.gameMode)) ? 10 : body.color; // blue
                    // ==============================================================
                            ...
                            let visible = nearby.map(function mapthevisiblerealm(e) {
                                if (e.photo) { 
                                    if (
                                        ...
                                        // Sandbox.
                                        // ===============================================================
                                        if (['wall', 'food', 'miniboss'].includes(e.type)){
                                            ...
                                            if (player.body && player.body.sandboxId === e.sandboxId){
                                                sameRoom = true;
                                            }
                                            else if (player.socket && player.socket.sandboxId === e.sandboxId){
                                                sameRoom = true;
                                            }
            ...
            const broadcast = (() => {
                ...
                const getminimap = (() => {
                    ...
                    let cleanmapreader = (() => {
                        ...
                        // Define the function
                        return (room.gameMode === 'ffa' || room.gameMode === 'sandbox') ? 
                ...
                const getleaderboard = (() => {
                        ...
                        let process = (() => {
                            ...
                            function getfull(entry) {
                                return [
                                    ...
                                    // Sandbox.
                                    // ================================
                                    entry.sandboxId ? entry.sandboxId : -1,
                                    // ================================
                                ];
                            }
                            return {
                                normal: entry => {
                                    ...
                                    if (lb != null) {
                                        ...
                                        leaderboard['_' + id] = {
                                            ...
                                            // Sandbox.
                                            // ================================
                                            sandboxId: entry.sandboxId ? entry.sandboxId : -1,
                                            // ================================
                                        };
                                        // Return it for broadcasting
                    ...
                    return () => {
                        ...
                        // ==========================================================================
                        // Sandbox. Leaderboard does not work properly.
                        // ==========================================================================
                        // Send all the players scores and filter on clients based on sandbox id.
                        while (list.getCount()){
...
var gameloop = (() => {
    ...
    let collide = (() => {
        ...
        return collision => {
            ...
            // Sandbox.
            // ==========================================================
            if (!['wall', 'food', 'miniboss'].includes(instance.type) && 
                !['wall', 'food', 'miniboss'].includes(other.type)){
                if (instance.sandboxId !== other.sandboxId){                
                    return 0;
                }
            }                
            // ==========================================================
